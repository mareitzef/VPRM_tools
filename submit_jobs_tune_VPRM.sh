...
for folder in "${folders[@]}"; do
    ...
    cat <<EOF >"job_${folder}.sh"
#!/bin/bash
#SBATCH --job-name=tune_VPRM_${folder}
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --mem-per-cpu=4G

module load python
srun python your_script.py -p "$base_path" -f "$folder" -i "$maxiter" -m "$opt_method" -v "$VPRM_old_or_new"
EOF

    # Submit the job to the cluster
    sbatch "job_${folder}.sh"
done
